...
trigger BT_AT_PPS057DX_RefundFileResponse on BT_CO_PPS_RefundFileResponse__c (before insert, before update) 
{
    ...
    for(BT_CO_PPS_RefundFileResponse__c so : recordsList) {
        ...
        if (StringUtility.validate(vCustomerAccount).equalsIgnoreCase('0')) accountNumberSet.add(vCustomerAccount.trim());
        System.debug('PPS057DX: Z1 '+vCustomerAccount);
    }
    ...
    for (Account acct : accountsList) {
        ...
        System.debug('PPS057DX: Z2 '+acct.AccountNumber);
    }   
    ...
    for(Zuora__Refund__c refund : refundslist) {
        ...
        String vAmount = null;
        if (StringUtility.validate(dPaymentAmount).equalsIgnoreCase('0')) vAmount = dPaymentAmount.setScale(2).toPlainString();
        if (StringUtility.validate(dPaymentAmount2).equalsIgnoreCase('0')) vAmount = dPaymentAmount2.setScale(2).toPlainString();
        ...
        System.debug('PPS057DX: Z3 '+searchString);
