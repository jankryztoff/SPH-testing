...
trigger BT_AT_ARM069DX_OutstandingAmountInterface on BT_CO_ARMS_OutstandingAmountInterface__c (before insert, before update) 
{
    ...
    /*for(Zuora__CustomerAccount__c zBillingAccount : [SELECT Id, ARMS_Account_Number__c, Zuora__Account__r.Account_Auto_Number__c FROM Zuora__CustomerAccount__c WHERE ARMS_Account_Number__c IN :inputARMSAccountNumberSet OR Zuora__Account__r.Account_Auto_Number__c IN :inputSynonymSet])*/
